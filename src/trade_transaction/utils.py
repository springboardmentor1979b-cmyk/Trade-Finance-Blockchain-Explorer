...
class TransactionUtils:
    ...
    def calculate_transaction_summary(
        ...
        # Status breakdown
        status_breakdown = {
            'pending': sum(1 for t in transactions if t.status == TransactionStatusChoices.PENDING),
            'in_progress': sum(1 for t in transactions if t.status == TransactionStatusChoices.IN_PROGRESS),
            'completed': sum(1 for t in transactions if t.status == TransactionStatusChoices.COMPLETED),
            'disputed': sum(1 for t in transactions if t.status == TransactionStatusChoices.DISPUTED)
        }
    ...
    def get_pending_transactions_count(session: Session, user_id: int) -> int:
        ...
        """
        query = select(func.count(TradeTransactions.id)).where(
            (
                (TradeTransactions.buyer_id == user_id) | 
                (TradeTransactions.seller_id == user_id)
            ) &
    ...
    def get_recent_transactions(
        ...
        query = select(TradeTransactions).where(
            ...
            TradeTransactions.created_at.desc()
        ).limit(limit)
